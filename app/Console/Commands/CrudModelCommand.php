...
class CrudModelCommand extends GeneratorCommand
{
    ...
    protected function replaceSelectFields(&$stub)
    {
        ...
        if ($fields) {
            ...
            foreach ($fieldsArray as $index => $item) {
                ...
                $name = $itemArray[0];
                $type = $itemArray[1];
                if (in_array($type, ['select', 'enum']) && isset($itemArray[2])) {
                    $optionsJson = str_replace("options=", "", $itemArray[2]);
                    $optionsArr = json_decode($optionsJson, true);
                    $optionsAsPhp = '';

                    foreach ($optionsArr as $k => $v) {
                        $v=strtolower($v);
                        $optionsAsPhp .= PHP_EOL . "'$k'=>'$v',";
                    }
                    $fieldsTxt .= PHP_EOL . "'$name'=>[$optionsAsPhp],";
                }
            }
            $fieldsTxt .= '];';
            $fieldsTxt = str_replace(",];", PHP_EOL . "];", $fieldsTxt);
            $fieldsTxt = str_replace(",]", PHP_EOL . "]", $fieldsTxt);
            $stub = str_replace('{{selectFields}}', $fieldsTxt, $stub);
        }
        return $this;
    }
...
EOD;
        ...
        return $ret->replaceSelectFields($stub)->replaceClass($stub, $name);
    }
