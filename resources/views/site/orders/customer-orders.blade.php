...
@section('content')
    ...
    <div class="row justify-content-center pg-content">
        <div class="col-md-6 col-12 rifas {{ $config->tema }}">
            ...
            <?php
            /** @var \App\Models\Participant $reserva */
            ?>
            @foreach ($reservas as $reserva)
                    <?php

                    /** @var \App\Models\Product $rifa */
                    $rifa = $products[$reserva->id];
                    $order = $reserva->order()->select("uuid")->first();
                    ?>
                    ...
                    <div class="card-body">
                        <div class="row align-items-center row-gutter-sm">
                            ...
                            <div class="col ps-2"><small
                                ...
                                <div class="compra-title font-weight-500">{{ $rifa->name }} - #{{$reserva->id}} </div>
                                <small
                                ...
                                @else
                                    ...
                                    @if ($reserva->qtdReservados() > 0)
                                            <?php
                                            $orderUuuid = $order->uuid;
                                            ?>
                                        <br>
                                        <a href="{{ route('site.checkout.pay', ['uuid'=>$orderUuuid]) }}">
