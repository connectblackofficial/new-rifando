...
<div class="form-group {{ $errors->has('telephone') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="telephone" type="text" id="telephone" value="{{ isset($user->telephone) ? $user->telephone : old('telephone')}}">
...
<div class="form-group {{ $errors->has('afiliado') ? 'has-error' : ''}}">
    <label for="afiliado" class="control-label"><?= htmlLabel('afiliado') ?></label>
    <select name="afiliado" class="form-control" id="afiliado">
    @foreach (json_decode('{"0":"Yes","1":"No"}', true) as $optionKey => $optionValue)


        <option value="{{ $optionKey }}" {{ (isset($user->afiliado) && $user->afiliado == $optionKey) ? 'selected' : ''}}>{{ htmlLabel(strtolower($optionValue))}}</option>
    @endforeach
</select>
...
<div class="form-group {{ $errors->has('pix') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="pix" type="text" id="pix" value="{{ isset($user->pix) ? $user->pix : old('pix')}}">
    {!! $errors->first('pix', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('cpf') ? 'has-error' : ''}}">
    <label for="cpf" class="control-label"><?= htmlLabel('cpf') ?></label>
    <input class="form-control" name="cpf" type="text" id="cpf" value="{{ isset($user->cpf) ? $user->cpf : old('cpf')}}">
